// TODO[ASK] §1 — раскладка 35 фото по лотам предварительная; подтвердить с владельцем.
...
export type BrickColor = 'dark' | 'red' | 'cognac';

export type Lot = {
  id: string;             // 'vb-14'
  number: number;         // 14
  line: 1 | 2;            // 1 = у воды, 2 = у дороги
  positionInLine: number; // 1..11 (в линии; line2 отображается в обратном порядке)
  status: LotStatus;
  brick?: BrickColor;
  brickRu?: string;
  shortLabel?: string;
...
export const village = {
  ...
  galleryPhotos: [P(26), P(25), P(24), P(1), P(12), P(11), P(19), P(32), P(7), P(4), P(28), P(3), P(35)],
  ...
  engineering: {
    ...
    // TODO[ASK] §4 — уточнить провайдера интернета
    internet: undefined as string | undefined,
  ...
  excluded: [
    ...
    { label: 'Интернет', detail: 'уточняется' }, // TODO[ASK] §4
  ...
  lots: ([
    ...
    },
    {
      ...
      status: 'available',
      brick: 'dark',
      brickRu: 'тёмно-коричневый кирпич',
      shortLabel: '2-я линия · западный край',
      distanceToPierM: 110,
      photos: [P(1), P(2), P(3), P(4), ...INTERIOR_PHOTOS.slice(0, 2)],
      description:
        'Крайний западный дом второй линии — большой участок открывается на три стороны. Тёмно-коричневый облицовочный кирпич и монолитная плита в основании: дом не просел за три зимы, которые уже позади. В системе всё работает — газовый котёл запущен, 15 кВт подключены, счётчики опломбированы.\n\nДо пирса на Волге пешком 110 метров — это меньше двух минут. Вторая линия на 20% дешевле первой, а тишины здесь даже больше: от уличного движения вас отделяет ряд соседних домов. Черновая отделка — стены газоблока ровные, под штукатурку без дополнительной подготовки.\n\nЦена в договоре — 14,3 млн ₽, торга нет. Гарантия на строительно-монтажные работы 2 года.',
    },
    {
    ...
    },
    {
      ...
      status: 'available',
      brick: 'cognac',
      brickRu: 'коньячный кирпич',
      shortLabel: '2-я линия · восточный край · максимум солнца',
      distanceToPierM: 140,
      photos: [P(11), P(15), P(16), P(17), P(21), ...INTERIOR_PHOTOS.slice(4)],
      description:
        'Второй от восточного края — максимум утреннего солнца на участке, гостиная смотрит на восток. Коньячный облицовочный кирпич — тёплый янтарный оттенок, которого в этом посёлке больше не будет: все три дома с разными цветами, этот самый редкий.\n\nДом прошёл три зимы без единой протечки и трещины. Газ в трубе, котёл греет батареи, 15 кВт заведены. Стены газоблока подготовлены под штукатурку, перекрытия на высоте 3 м. До пирса — 140 метров по дорожке посёлка.\n\nСвободных домов в первой линии уже нет — это лучшее из того, что осталось. Цена — 14,3 млн ₽, фиксированная.',
    },
    {
